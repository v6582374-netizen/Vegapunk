"""Source configuration for the independent Prompt Translation Instruction."""
...
def read_prompt_translation_instruction(path: Path) -> dict:
    with source_configuration_transaction():
        return _document(path)


def save_prompt_translation_instruction(path: Path, instruction: str) -> dict:
    if not instruction.strip():
        raise ValueError("Prompt Translation Instruction must not be empty")
...
def _document(path: Path) -> dict:
    ...
    if not isinstance(values, dict) or not isinstance(values.get("instruction", ""), str):
        raise ValueError("Prompt Translation Instruction must contain a string instruction")
