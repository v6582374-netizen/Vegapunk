...
flowchart TB
    ...
    subgraph Execution["实验与产物"]
        ...
        Backends["experiments_utils_*<br/>codex / qwen_code / openhands"]
...
flowchart TD
    ...
    O --> P["run_codex_experiment / run_qwen_code_experiment / run_openhands_experiment"]
...
flowchart TD
    ...
    Backend -- qwen_code --> Qwen["run_qwen_code_experiment()"]
    ...
    MCTSFlag -- 否 --> Normal["perform_experiments_codex / qwen_code<br/>生成 run_0..run_N"]
...
| MCTS 实验 | `CodexMCTSSearch`, `AiderMCTSNode` | 可选搜索代码方案树，用 UCT 选择、draft/improve、指标回传和 best node 更新；Qwen Code 复用同一 MCTS 合同。 |
