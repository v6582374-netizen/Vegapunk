...
An executed Experiment Run is complete and eligible to count as successful only when it has both its valid outcome artifacts, including `final_info.json`, and its required Run Report at `report/report.md`. If the outcome artifacts already exist but the Run Report is missing, Vegapunk keeps the same `run_N` and asks the same experiment backend to repair only the report from the Run's existing artifacts.
