...
An Experiment-Sourced Figure may either reuse an image already produced by an InternAgent Experiment or be newly rendered from exact structured values recorded in the Selected Research Candidate's Experiment artifacts. The latter is a Presentation Transformation: it may select, order, label, and map recorded values to visual channels, but it may not create a new measurement, metric, aggregation, fit, smoothing operation, interpolation, statistical inference, selection judgment, or repaired value.
