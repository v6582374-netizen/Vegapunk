...
InternAgent currently requires sci-task agents to save figures and reference them from `report/report.md`, but it does not explicitly require a non-empty Markdown alt text or a separate publication caption, and no Run Report validator enforces either. Until InternAgent establishes an explicit caption artifact contract, PaperOrchestra may author the publication caption for an eligible Experiment-Sourced Figure.
...
This is an explicit temporary exception, not a reason to treat alt text and caption as the same concept. A later InternAgent caption contract can replace PaperOrchestra-authored captions without changing ADR-0116's existing-image boundary.
