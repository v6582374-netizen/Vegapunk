...
PaperOrchestra content review and refinement will use the complete current LaTeX source together with text deterministically extracted from the compiled PDF. Layout review will use page images rendered from that PDF through the existing screenshot utility. The port will remove Gemini's provider-specific `application/pdf` binary input and will not add a native PDF-upload operation to Vegapunk's model interface.
