...
If the Paper Candidate Round contains exactly one successful Candidate Experiment, that candidate is selected directly without metric comparison. If it contains multiple successful candidates, the separate explicit-metric selection rule applies. If no completed round contains a successful candidate, Terminal Candidate Selection produces no selection context; under ADR-0092, PaperOrchestra still constructs the Paper from the complete Research Draft and launch evidence.
