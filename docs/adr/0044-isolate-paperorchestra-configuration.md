...
The file does not duplicate the primary text provider, model, or credential settings. PaperOrchestra receives that model through Vegapunk's existing configuration and `ModelFactory`; ADR-0097 defines the separate image-only provider and environment variable. `launch_discovery.py` loads the fixed default PaperOrchestra configuration path, while a standalone historical-run command may explicitly override that path.
...
- Add PaperOrchestra settings to `config/default_config.yaml`. Rejected because writing configuration can remain additive and isolated from Vegapunk's existing configuration contract.
- Duplicate model configuration in the new file. Rejected because PaperOrchestra must use the same unified model instance as Vegapunk rather than create a second provider path.
