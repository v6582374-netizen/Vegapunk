...
- Overlay the upstream tree onto the Vegapunk repository root. Rejected because both projects own root-level files and package names, which would erase the source boundary and create ambiguous configuration and dependency ownership.
- Keep PaperOrchestra as a Git submodule. Rejected because the replacement source and its local adaptations must be present and reviewable in the Vegapunk repository rather than stored as state belonging to a second repository.
