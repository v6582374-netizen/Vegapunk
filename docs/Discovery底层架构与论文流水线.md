# Discovery：底层架构与论文流水线

## 从一封任务信到一份可复核的发现与论文

> 本书写 Discovery 的底层架构，也写它如何把已经落盘的研究材料交给 PaperOrchestra。
>
> 故事在 Discovery 写下 `discovery_summary.json` 后继续向前：同一个 Launch 把已确认的发现材料交给 `paper_orchestra_runs/paper`，论文工坊再留下 TeX、PDF 和自己的运行记录。这样既把 Discovery 的独立恢复边界讲清楚，也不把论文阶段误写成模型的一次长回复。
...
发现流程有一个很容易被误解的门槛。很多人一看到 `launch_discovery.py` 末尾调用了论文阶段，就以为 Discovery 的每一行都在为论文服务。本书把两段因果链接起来，却不把它们揉成一团：Discovery 负责创建 Launch、恢复轮次、选择候选、运行实验、更新 baseline、生成经验并写出 `discovery_summary.json`；PaperOrchestra 在这个事实横截面上开始自己的 Run。

书中不展开上层交互、服务协议或状态投影的设计逻辑，只保留底层真正影响输入、状态、产物、恢复和证据归属的部分。PaperOrchestra 也只讲它如何被启动、读取哪些原生产物、把结果写到哪里，以及失败为什么不改写 Discovery 的事实；章节、提示词和版式细节留在论文工坊自己的文档里。

## 全书的总地图


主线可以这样记：命令行先把任务领进工坊，启动器为本次运行留下输入快照；外层 Round 驾驶 Session，Session 让想法经过生成、批评、查证、演化和排序；被选中的方向进入方法开发和实验工作台；实验结果回到基线、记忆与下一轮，最后由 `discovery_summary.json` 把事实封存；封存之后，同一个 Launch 再把原生材料交给 PaperOrchestra，留下 `paper` Run 的论文产物。换成一条连续的因果链，就是“确认输入 → 建立会话 → 加厚候选 → 隔离验证 → 比较指标 → 选择基线 → 封存证据 → 论文交接”。
...
配置文件像工坊墙上的节拍器。`default_config.yaml` 里，Discovery 的典型默认值是：MAS 内部最多四次迭代，保留五个 top ideas，开启 top ideas 演化，外部数据工作区按 `max_concurrent_tasks` 控制；外层完整 Discovery 轮数为十，loop mode 为 `incremental`；实验最大运行次数为两（`run_0` 是基线，`run_1` 到 `run_N` 是改动后的尝试），实验最多四个并行槽位。模型请求的硬门槛另由 `OrchestrationAgent` 的 `MAX_CONCURRENT_LLM_TASKS=2` 守住，搜索任务则由 `MAX_CONCURRENT_SEARCH_TASKS=10` 单独限制。

这些数字是默认节奏，不是科学定律。`_main` 从命令行或配置读取 `loop_rounds` 与 `loop_mode`；`OrchestrationAgent.__init__` 从 `workflow` 段读取 `max_iterations`、`top_ideas_count`、`top_ideas_evo` 和 `max_concurrent_tasks`，同时把模型和搜索的更窄并发放在代码常量里；`ExperimentRunner` 再读取实验段的并发和后端设置。每个层级只消费自己负责的字段，避免把“外层十轮”误当成“Session 内四轮”，也避免把外部数据并发当成 provider 并发。
...
第二个站是输入站。任务目录、任务类型、参考代码和 prompt 路径在这里被固定。第三个站是节奏站，读取 `loop_rounds` 与 `loop_mode`。第四个站是提前结束检查：如果 `start_round > loop_rounds`，说明 Discovery 轮次已经完成，系统不再重跑想法和实验，而是直接检查同一 Launch 是否需要进入 Paper Handoff。
...
统一运行时还有一个更底层的意义：模型请求、工具回路和实验分析都从同一条 provider 供电线上经过。请求可以沿 response chain 继续，恢复和测试也可以注入一个假的 runtime 替换真实 provider；代码不需要在每个角色里复制一套“凭据、重试、提供方选择”的逻辑。长过程观察由各阶段自己的日志和结构化产物承担，不再依赖一个额外的全局捕获层。
...
创建一个 Agent 时，Factory 先检查类型是否注册；未注册就抛出 `ValueError`。随后用角色与当前模型配置生成缓存键。如果缓存命中，返回已有实例；否则从统一运行时创建模型、实例化 Agent 并写入缓存。Factory 只负责角色和运行时的装配，不附加额外的全局记录钩子，也不替角色偷偷建立另一套模型客户端。
...
## 第十七章　记录不会另起一条历史：结构化产物各自守门

当前 MAS 不再设置一条全局草稿带，把模型、工具和运行器的长观察统一追加在一起。结构化状态回到 `WorkflowSession` 和 `ideas.json`，实验事实回到候选工作台、`run_N/final_info.json` 和日志，论文阶段也只读取 Launch 中已经存在的原生产物。

这不是把过程抹掉，而是把过程放回真正拥有它的房间。模型—工具往返由 `ModelToolLoop` 返回结果和终止原因，外部执行器保留自己的 stdout、stderr、traceback 与运行日志，轮次记录保存成功和失败的压缩事实。没有一只全局记录盒替所有阶段宣称“我知道发生了什么”。

这样分开以后，恢复时先读结构化事实，再按需进入日志和运行目录：Session 告诉值班员流程站在哪个门牌，Idea 告诉他候选变厚到哪一步，`final_info.json` 告诉他指标是否可比较，日志负责解释中途发生过什么。某段过程记录缺失，不应被补写成模型的隐含推理；某个结构化字段缺失，也不能用一段日志里的礼貌话代替。
...
`WorkflowSession` 是更大的账本。它包含 `id`、`task`、`ideas`、`iterations_completed`、`max_iterations`、`state`、`feedback_history`、`top_ideas`、`tool_usage`、`started_at`、`completed_at` 和 `error`，还会记录是否进入 `method_phase`。Session 不等于所有日志；它是流程能否继续的压缩状态，阶段日志、工具结果和候选工作台各自承载更长的事实。
...
回调失败不会反向改变状态。状态回调和日志收集器只是旁观者，研究流程本身仍由 Session 和持久化事实驱动。这个隔离让记录层可以短暂出错，而不让候选消失。
...
生成阶段只写入 Idea 的最初字段。它不会把模型的 `reasoning` 放进 `Idea`，也不会把工具返回的所有原文塞进 `baseline_summary`。GenerationAgent 的结果里有 metadata 和 reasoning，但 OrchestrationAgent 只取 `hypotheses` 和 `baseline_summary`。这是一种有意的压缩：Session 保存流程需要的结构化事实，工具结果和阶段日志分别保留可追查的过程。
...
当前 MAS 还多了一道由 Connector 看守的证据门。需要结构化外部数据的 Idea 会得到自己的工作区，Connector 必须写出 `idea_evidence_manifest.json`；系统先校验清单、来源、请求、时间和文件是否真的位于该工作区，再把条目接入 `Idea.evidence`。清单不完整时，证据不被半信半疑地收下，而是在 `acquisition_events` 里留下失败或 `invalid_manifest` 事实。

只有 Connector 返回 `open_web_evidence_gate=true`，Web Evidence 才会被叫进来补资料。它不是默认的第二条搜索路，也不能绕过同一份清单校验；没有新条目、Agent 不可用或调用失败，都记录 acquisition event 而不伪造 evidence。这个门控把“需要外部证据”和“可以打开 Web Evidence”拆成两个明确判断。
...
EvolutionAgent 的输出 schema 要求每个子候选都有 `text`、`rationale` 和 `improvements`，同时给出整体 `reasoning` 和 `changes`。OrchestrationAgent 只把 text、rationale、baseline_summary、iteration 和 parent_id 写进新的 `Idea`；`improvements` 和 changes 留在 Agent 返回或阶段日志中，并不自动成为 Idea 字段。这意味着如果读者想追踪“子 Idea 改了什么”，要结合父子关系和演化事件，而不能只看子卡片的五个基本字段。
...
Discovery 的第一个终点在这里变得清晰：摘要写下最后一轮的事实，经验库可以继续被下一次运行读取，发现材料也在此刻冻结成论文交接的输入。接下来不是把摘要改写成结论，而是由同一个 Launch 启动一次独立的 PaperOrchestra Run；论文阶段可以失败，但不能回写已经封存的 Discovery 轮次。

# 第七幕　Launch 既是文件夹，也是一个可被观察的承诺

## 第五十一章　从输入快照到可启动的版本

Discovery 的底层入口只有一个问题：这次 Launch 究竟看见了哪一份任务输入。普通任务从任务目录复制 `prompt.json`，科学任务由 `normalize_sci_task` 把 `task_info.json` 和清单折叠成合成提示；随后启动器把它们放进 Launch，形成这次运行自己的输入快照。Launch 认领的不是某个进程里暂时存在的字符串，而是一份可以在恢复时重新读取的文件。

这个过程像实验室里的签字。任务目录是原材料，标准化是把它整理成 Task 所需的描述、领域、背景和约束，Launch 内的 prompt 是签字后的版本。只要任务输入在签字后被改过，就应该建立新的 Launch 或明确生成新的提示备份；不能让恢复程序把“现在的目录”冒充成“当时的任务”。

标准化失败也有明确的位置。`task_info.json` 缺失、科学清单格式无法读取或普通任务找不到 `prompt.json` 时，入口在门口失败，不生成半结构化 Task。Discovery 不让模型承担输入校验的最后责任，因为输入边界一旦模糊，后续所有实验、恢复和摘要都会失去可解释性。

当 Launch 被选中时，输入快照至少要能回答任务身份、任务类型、prompt 来源、参考代码和配置从哪里来。原始任务目录仍然是事实源，Launch 内的副本则是本次运行的时间切片；两者同时保留，恢复时才能分辨“原始材料后来变了”与“本次运行本来就这样”。

## 第五十二章　开始一次 Launch 之前的最后盘点

入口盘点的是底层事实，而不是外部操作：任务目录存在吗，任务类型能确定吗，prompt 或科学任务材料可读吗，参考代码和输出位置是否已经固定，恢复路径是否真的指向一个 Launch。参数经过 `parse_arguments` 和 `_main` 整理后，才交给 Session、实验器和论文交接。

启动前有几项硬检查。任务目录不存在、prompt 缺失、科学任务说明不可读、恢复摘要损坏且目录也找不到完成轮次时，系统停在入口，不创建半个 Launch。错误越早停在输入边界，后面的 Agent 和实验器就越不必猜测自己拿到的是什么。

如果所有检查通过，系统会把当前配置、提示根和运行偏好复制或固定在 Launch 周围。这些快照属于 Launch，不会随着全局配置下一次编辑而改变。恢复时，运行器优先读取 Launch 自己的事实；即使模型目录、实验偏好或提示已经在仓库里变化，旧 Launch 仍然沿用曾经承诺的版本。

新启动与恢复都应当具备清楚的身份和幂等边界：同一个 Launch 恢复请求不能凭重试再开第二套 Session，同一轮也不能因为进程重新进入而重复写一份互相矛盾的摘要。底层锁、目录身份和结构化完成标记共同守住这条边界；命令行本身不需要知道任何上层协议。
...
底层观察的任务不是替流程做决定，而是把已经落盘的事实重新读出来。Session JSON 负责会话状态，轮次记录负责外层统计，运行器标记和目录产物负责实验进度；读取时先核对这些来源，再形成一个“此刻可以相信什么”的快照。

观察不能只信内存。进程重启后，内存里的对象可能已经消失；外部后端也可能在编排器不知道时退出。恢复或诊断时，应从 Launch、Session、摘要、运行器标记和最终产物重新对账。证据不足时保守地停在等待或需要处理，而不是把一个没有 `final_info.json` 的目录写成 completed。

数值也要经过底层边界检查。非有限数字、缺失指标和格式错误的 JSON 不能直接进入 performance 或下一轮选择；它们应被标成缺失、警告或失败。可观察性的第一责任不是让故事顺滑，而是保证下一步判断不会建立在伪造的精确度上。

日志仍然重要，但它是解释材料，不是状态源。日志可以告诉读者命令、堆栈、超时和后端细节；结构化 Session、结果和摘要才负责恢复、统计与条件判断。两者相互照应，却不能为了方便互相冒充。

## 第五十六章　产物只认自己的 Launch 门牌

底层产物合同首先要求归属清楚。Launch 根目录里的 prompt、Session、轮次记录、候选工作台、日志和摘要，都只能解释这一次运行；共享经验库和任务级图谱虽然可以被多个 Launch 读取，却不能把当前运行的半成品写成别人的历史。

路径解析也属于底层合同。任何需要读取的文件都要先规范化，再确认仍然位于选定 Launch 或候选工作台的根目录之下；`../`、绝对路径和逃出根目录的符号链接都不能成为证据入口。这个检查不是上层装饰，而是为了让一条路径永远只能指向它所属的运行。

产物还要区分事实层级。`final_info.json` 是指标来源，`notes.txt` 是候选门牌，日志和 traceback 是诊断材料，`discovery_summary.json` 是恢复横截面。它们可以被同一位读者打开，却不能互换语义。找到一个文件，不等于已经找到可以支持某个结论的证据。

两个 Launch 可以处理同一个任务，也可以有同名候选；归属必须由 Launch ID、Session ID、Idea ID 和目录层次共同确定。测试应该验证“拿 A Launch 的路径去解释 B Launch”会被拒绝或标记为不一致，而不是因为名字相似就悄悄拼接两段历史。

## 第五十七章　状态、失败和终点的三角关系

一个 Launch 的状态不只是 `running` 或 `completed`。它还要说明当前是否仍有可恢复材料、摘要是否完整、外部运行器是否已经退出，以及下一步应该继续哪个阶段。状态由持久化事实推导，而不是由最后一行日志或某个进程对象单独决定。

恢复和重试都必须有明确归属。已经完成的轮次不能因为重新进入入口就再写一份结果；正在运行的候选不能因为一次读取失败就被当成新的实验；同一个 Launch 的恢复应回到同一目录和同一 Session。锁、目录身份和完成标记共同防止历史被重复书写。

终态是只读的承诺。Discovery 完成后，摘要、Session、实验和错误仍可读取，但不能把已经封存的轮次改成另一种历史。失败终态也要保留工作台和诊断材料；恢复只能从明确允许的边界继续，不能用“重试”把旧失败擦掉。

如果摘要显示轮次已完成但配置或关键产物缺失，系统应把它标成材料不完整，而不是把缺口藏起来。如果运行器标记与摘要不一致，先对账再决定状态。状态是事实的投影，不是脱离文件和进程的标签。

## 第五十八章　断点恢复的两条时间线

底层恢复有两条相邻但不同的时间线。外层 `--resume` 关注 Discovery Round：它扫描 `discovery_summary.json`、Session 目录和最佳代码路径，决定从哪一轮继续；内层 Session 恢复关注 `WorkflowSession` 的门牌：它要知道候选停在生成、反馈、外部数据、方法还是错误。

两条时间线最终在 Launch 目录汇合，却不能互相冒充。外层知道某一轮是否完成，但不替 Session 猜当前 Idea；Session 知道自己停在哪个状态，却不替外层选择下一轮 baseline。每层恢复自己负责的状态，再通过摘要、Session JSON、运行器标记和目录结构交换事实。

进程重启时，内存中的等待不会被当作可恢复的事实。恢复依赖持久化的输入快照、配置快照、运行器标记、Session JSON 和候选工作台。没有这些材料，系统宁愿停在需要处理，也不会凭一个“曾经在运行”的状态继续发送模型请求。

一个被中断的 MAS Session 可能停在 `AWAITING_FEEDBACK`、外部资料或实验完成之前。恢复程序首先读取最后一次已保存的 Session；如果状态处在明确的等待点，它可以从该点继续；如果状态在错误房间且错误没有解决，恢复只能重试被允许的阶段，不能假装已经完成。

## 第五十九章　时间线由许多账本拼起来

当前系统没有一条全局草稿时间线。运行器保留自己的调用参数、最后消息、JSONL、stdout、stderr 和 traceback；`WorkflowSession` 保存状态与 Idea；候选工作台保存 `notes.txt`、代码、运行目录和最终信息；Launch 摘要保存外层的压缩横截面。它们各自写自己真正拥有的事实。

并发执行时，关键不是让所有记录排成一条漂亮的时间线，而是让每条记录都带着归属：哪个 Launch、哪个 Session、哪个 Idea、哪个 `run_N`。这样，两个候选即使同时完成，也不会因为日志行交错而互相冒充。真正的时间顺序再由时间字段、轮次编号和目录关系重建。

执行器只有在进程成功结束且找到约定的最终消息时才返回成功；进程失败却吐出“看起来像完成”的文字，仍然必须被记为失败。这个合同让操作系统信号和模型礼貌话各自守住边界，不需要一份额外的长篇草稿来替它们裁决。

过程记录可以很长、包含重复和局部错误；结构化状态必须稳定、可恢复。把两者混成一个文件，会让过程太重、事实太模糊。好的时间线不是一条巨大的文本，而是一组可以互相核对的账本。

## 第六十章　从暂停点继续，而不是从头再猜

一个好恢复故事应该能回答四个问题：上一次在哪里停，已经写下什么，接下来应该执行哪个阶段，哪些外部条件仍然有效。Session JSON 提供状态、Idea 和计数；Launch 摘要提供轮次与基线；配置快照提供模型和偏好；候选工作台、阶段日志和运行器记录提供停下前的上下文。
...
如果摘要显示 Round 1 完成、Round 2 有目录却没有完整结果，`_scan_completed_rounds` 只能把 Round 1 算作完成。目录存在不是成功的同义词，必须有轮次结果或可验证的摘要字段。正因为扫描严格，恢复才不会把半成品重复当成新一轮。

## 第六十一章　输入快照和产物快照的相互照应

输入快照说“当时研究的是什么”，产物快照说“当时留下了什么”。前者包括任务目录、prompt、任务类型、参考代码和配置；后者包括摘要、Session、实验、日志和论文交接目录。恢复时只读产物而不核对输入，可能把一份旧结果误套在新任务上；只读输入而不核对产物，又可能重复已经完成的工作。

任务路径、Launch ID、Session ID、Idea ID 和目录层次把两者连接起来。任务目录后来可以变化，Launch 内的 prompt 仍保留当时的输入；最佳代码路径后来可以前移，摘要仍记录它为何成为下一轮 baseline。连接它们的不是一个应用指纹，而是一组能在文件系统中互相验证的身份和快照。

输入和产物的清理也要尊重归属。一个 Launch 建立失败时，不能顺手删除仍被其他运行引用的共享经验；一个候选失败时，也不能因为没有最终指标就删掉它的工作台。清理的目标不是把目录变漂亮，而是避免历史出现无法解释的空洞。

## 第六十二章　失败类型的四层地图

第一层是输入失败：任务目录不存在、prompt 缺失、科学任务材料不可读、任务类型无法确定。这一层的失败应该阻止运行启动，因为系统还没有一个可信的研究问题。
...
想象一个没有历史经验、没有旧 Launch 的空目录。任务目录先准备好 prompt 或科学任务材料，命令行入口认出任务类型，建立 Launch，复制或生成 prompt，读取当前配置，决定第一轮的 baseline。

IdeaGenerator 建立 Interface 和 Session，背景调研可能成功也可能降级；生成员摊开候选，反思员标出风险，资料员补证据，演化员产生孩子，排序员选出 top ideas。状态和 Idea 被反复写入记忆，阶段日志接住过程，最后方法开发把候选变成可执行委托单。此时还没有任何实验指标，只有待验证的方案。
...
最后一轮不再寻找下一条接力棒。外层统计所有轮次和 Session，把实验后端、模式、数量、成功和失败写进 `discovery_summary.json`。随后 `_handoff_to_paper_orchestra` 只把这个已经落盘的 Launch 交给论文服务；Discovery 的过程不靠一段“模型说它完成了”的话，每个阶段都留下可以核对的物证。

# 第八幕　把全景拆成可以检查的契约

## 第六十六章　入口的三次盖章

底层入口至少要盖三次章：第一次确认任务目录和任务类型，第二次确认 prompt 或科学任务已经被标准化，第三次确认 Launch 目录、配置和输入快照已经落盘。任何一枚章缺失，研究都没有资格进入 Session 或实验工作台。

第一枚章回答“手里的任务是什么”；第二枚回答“MAS 能否把它解释成 Task”；第三枚回答“恢复时从哪一份文件继续”。这三件事分开，意味着入口可以明确拒绝半成品，也意味着后续阶段不必用模型补写缺失事实。

实验输入同样遵循顺序。基线代码、任务类型、数据和输出目录先固定，候选工作台再从它们复制；如果某个目录在复制时失败，失败留在入口或工作台，不让一份不完整的状态悄悄进入指标比较。

## 第六十七章　锁不是为了让系统变慢

启动、恢复、轮末封存和产物读取都可能同时发生。底层目录锁或等价的事务边界把“加载磁盘、检查标记、修改索引、持久化事实”放在一起。锁内的代码不追求做完所有耗时工作，而是保证关键事实不会被两个执行路径交错写成互相矛盾的版本。

例如一个恢复进程和一个仍在运行的 worker 同时触碰同一 Launch，第一方先确认目录标记并登记状态，第二方在锁内看到同一事实后只能继续或等待，不能再创建一套 Session。若没有这个锁，两个路径都可能通过“尚未完成”的检查，随后各自写摘要或改 baseline，用户只会得到一条难以还原的历史。

共享资源也有清理窗口。假如一轮已经把经验或来源写入共享目录，随后写 Launch 摘要时失败，锁保证清理动作可以判断这些材料是否已被其他运行引用。没有锁，清理可能把另一个 Launch 正在使用的经验删除，造成一个很难在日志中还原的随机损坏。

锁的存在不意味着所有请求串行到最后一毫秒。外部运行器的长任务在锁外执行，状态变化通过标记和对账重新进入锁内。这样，系统既能保持文件索引的一致，又不让一个长时间实验阻塞用户查询其他 Launch 的状态。

## 第六十八章　幂等键背后的请求指纹

幂等身份只说明“这组重试属于同一组意图”，不能单独说明意图是什么。Discovery 因此把 Launch、Session、Round 和候选工作台身份按稳定顺序记录。相同恢复身份可以重放或继续；同一目录若带着不同任务、不同 prompt 或不同 baseline，就必须停在冲突边界，因为这通常意味着调用方把一次运行误当成了另一件事。

不同动作的幂等边界也不同。恢复主要绑定 Launch 和已完成轮次，实验重试绑定候选工作台与新的 `run_N`，论文交接绑定 Launch 与固定的 `paper` Run。它们不能共享一个模糊的“重试”开关；每种动作都要定义自己不可变的意图。
...
外部运行器有自己的生命周期：进程可能已经退出，日志可能已经写完，编排器也可能在不知道时重启。内存索引不能永远相信自己上次写下的 `running`。每次需要生成状态观察时，恢复逻辑都应重新读取磁盘和运行器标记，把“进程已完成但索引仍显示运行”的差异对账成最新状态。

对账不是重新执行任务。它只检查持久化的标记、摘要、Session 和已知产物，确认某个状态变化是否已经发生。若证据不足，状态保持保守，例如继续停在需要恢复，而不是猜测任务成功。宁可让值班者多看一次“正在确认”，也不应把一个没有最终结果的运行写成 completed。

状态观察还会计算允许的下一步。对账发现进程已经结束且摘要完整，就进入封存；对账发现 Launch 仍有可恢复材料，就保留恢复入口；对账发现目录被破坏，就进入需要处理。下一步来自服务器之外的持久化事实，而不是某个进程内缓存的旧判断。

## 第七十章　模型目录是每次运行的电路图

统一运行时不是把一个模型字符串传给所有地方。模型目录区分主文本模型、结构化输出能力、图像能力和嵌入能力，并记录它们的规范身份。Launch 开始时把目录物化成自己的快照，MAS 和实验器共享这一快照，避免一个阶段使用了不同于另一个阶段的模型。

这解释了为什么配置里可能同时出现人类可读的模型拼写和运行时的规范身份。前者帮助配置者定位，后者服务于适配器路由；两者在启动时被映射，之后以 Launch 的模型目录为准。恢复时如果只读取当前全局目录，就可能把同一 Launch 的前后两半接到不同模型上。
...
Discovery 的测试不是只验证函数返回值。它们把一组底层契约固定下来：启动必须拥有可读的任务输入，恢复必须从已完成轮次继续，产物路径不能越界，失败 Launch 必须保留历史，运行时必须被注入到正确的后端，论文交接必须使用固定的 Launch 输入。

测试中的临时目录模拟真实 Launch。它们创建 prompt、摘要、Session、候选工作台、`run_0` 和 `run_1`，然后通过入口、Runner 或论文交接函数读取和修改。这样的测试比只 mock 一个函数更接近真实边界，因为它会发现目录命名、文件复制和恢复扫描之间的缝隙。

一些测试故意写入“看起来像成功”的错误材料，例如进程失败却留下成功文本、最新运行没有指标、候选目录中存在过期报告。测试要求系统保留失败信号、选择最近的有效指标、避免用旧报告覆盖新事实。它们把经验中的坑变成可以持续运行的规则。

## 第七十三章　启动垂直切片测试了什么

垂直切片从任务目录开始：准备普通 prompt 或科学任务材料，启动 Launch，等待 MAS 和实验完成，读取 Session、候选工作台与摘要，最后验证同一 Launch 可以被 Paper Handoff 读取。它不依赖上层控制面，只需要走完底层输入、状态、实验和产物合同。

这条测试的价值在于它跨越了多个边界，却不依赖真正的模型或外部实验。它用假的 runtime、假的后端和临时 Launch，把输入快照、Session 持久化、`run_0`、摘要与论文交接的底层合同走一遍。只要某个环节把半成品当完成，垂直切片就会失败。

测试还验证冲突：同一 Launch 在恢复时不能凭不同输入继续写另一条历史，候选工作台不能被另一候选读取，产物路径穿越必须被拒绝，终态不能被改写。于是底层边界成为可执行的事实，而不是一份只写在说明里的约定。

## 第七十四章　状态与产物测试的安全边界

状态测试会递归检查持久化结果，确保非有限数字、损坏 JSON 和缺失最终信息不会被当成有效性能。它还会构造运行器已经退出但摘要尚未写完的场景，确认对账保持保守。底层观察的目标不是隐藏错误，而是把错误以稳定、可恢复的形式保存。

产物测试会从一个 Launch 的根目录读取文件，再尝试用另一个 Launch 的同名路径、`../` 路径、绝对路径和符号链接逃逸路径混入。只有选定根目录下、身份一致的文件能够被接纳。测试因此覆盖了“路径解析”和“Launch 身份绑定”两个不同问题。

失败运行也有专门的只读断言。运行器失败后，状态应转成 failed，历史、错误和工作台仍可查询；恢复只能从明确允许的边界开始，不能让失败历史被新结果覆盖。这样，失败是可研究的对象，而不是系统试图清理的垃圾。

## 第七十五章　阶段日志测试守住证据归属

当前测试验证阶段日志、Session JSON、候选工作台和 `run_N` 之间的归属。并发实验可以按任意顺序完成，但每个结果必须仍然指向正确的 Idea、工作台和最终信息；一个候选的日志不能被另一个候选的摘要吸收。

CodexRunner 一类执行器的测试仍然检查命令行参数和最后消息的关系：工作目录、模型、沙箱策略、网络设置和跳过仓库检查要正确传入；最后消息缺失时不能把 JSONL 事件流当作最终回答；进程返回失败时，即使文本里出现“完成”，也必须抛出错误。

这些细节让过程记录成为可靠的诊断材料。没有它们，后续读者可能把一个中途日志当成最终答案，把一段失败后的礼貌文字当成成功，或者在并发写入时看到无法归属的因果链。
...
读测试名往往比读一段长实现更快理解边界。诸如“completed discovery resumes into the paper handoff”“artifacts are bound to the selected launch”“failed launch is read-only history”“same runtime is shared between stages”这样的断言，都是 Discovery 不能轻易破坏的誓约。

这些誓约互相连接。只读历史要求失败证据仍然在目录里；产物绑定要求目录层次和 Launch ID 一致；共享运行时要求 `_main` 延迟构造但在需要时只构造一次；Durable resume 要求输入、配置和状态快照在进程重启后仍可被重建。

当未来有人修改实现时，最危险的不是某个函数变长，而是无意中改变这些关系。例如把状态直接换成内部临时字典、把结果列表重新排序后仍用位置解释、把配置改为每轮重新读取、把失败的工作台删掉。测试会在这些“看起来只是重构”的变化上发出警报。
...
有时用户已经在外部会议中整理好一份 Idea 文件，不希望再调用 MAS。`--skip_idea_generation` 让外层只跑一轮，并从 `--idea_path` 读取候选。它会先确认文件存在，再判断它是完整的 `hypotheses` 结构还是简单列表；若是完整结构，只抽取 `top_hypotheses` 指定的候选，避免把所有候选都送去实验。
...
产物清单本身就是这份导航的底层投影。它把 report、summary、工作台和 `paper` Run 放回各自的 Launch 根目录，阶段日志和指标仍由内部诊断读取。路径只作为证据坐标，不把一个目录里的调试细节误当成另一处的研究结论。
...
## 第九十一章　摘要写完，PaperOrchestra 接过接力棒

Discovery 的工作是把研究问题变成候选，把候选变成方法，把方法放进工作台，接受实验和证据的检验，再把结果和经验保存下来。`discovery_summary.json` 写完以后，队长没有把纸张散落在桌面上，而是把同一个 Launch 推到论文工坊门口：`_handoff_to_paper_orchestra` 调用 `run_paper_orchestra`，让论文阶段读取已经存在的原生产物。

这次交接只有一个固定的运行身份：`paper`。论文材料落在 `<launch_dir>/paper_orchestra_runs/paper/`，而不是另起一个无法追溯的临时目录。PaperOrchestra 先确认 Launch 有效、加载自己的配置、从同一模型目录建立运行时，再检查是否已经存在完整的终稿；如果已有 `final_paper.pdf` 与 `content_refinement_workdir/final_refined_paper.tex`，重入就复用这次 Run，而不是悄悄生成第二篇论文。

交接时，论文工坊拿到的是 Discovery 已经留下的材料：摘要、候选与实验目录、日志、报告、指标和可以定位的证据。候选选择是可选的上下文，不是让 Discovery 重新跑一遍的闸门；即使没有一个成功候选，PaperOrchestra 也只能根据实际存在的材料写作，不能凭空补出实验结果。Paper Input Bundle 是对原生材料的确定性投影，不替代它们，也不把模型新写的科学事实倒灌回 Discovery。

PaperOrchestra 自己在 `paper` Run 里完成论文阶段的工作，最终至少留下权威 TeX 与 PDF；运行日志、原始材料和阶段输出也留在同一目录，方便从论文回到 Discovery 的证据。论文可能在模型目录、上游进程、TeX 编译或最终产物校验处失败，但失败只属于这次 Paper Run，已经封存的 Discovery 摘要、轮次和实验结果仍然成立。

因此，真正的边界不是“Discovery 讲完，论文不重要”，而是“发现事实先封存，论文再消费事实”。这条单向交接让两座工坊可以相互衔接，却不互相改写：Discovery 不负责排版，PaperOrchestra 不负责重新定义 baseline；读者既能沿着故事走到最终 PDF，也能在论文失败时回到那份仍然可恢复的发现账本。
...
| launch | 一次隔离的外层运行 | 启动或恢复 | Launch 目录、摘要、论文交接 |
...
| observation | 对内部事实的可核对投影 | Session、摘要、运行器标记 | 恢复、诊断、论文交接 |
| artifact | 可定位的 Launch 产物 | Launch、Session、工作台 | 指标、恢复、论文材料 |
...
第一条是输入不变量：运行只能由已经确认、可读取且被 Launch 快照固定的任务输入启动。它保护研究问题不被缺失 prompt、错误任务类型或后来变化的目录偷换。
...
第五条是边界不变量：每份证据都只能从自己的 Launch、Session 或候选工作台读取，不能通过路径逃逸、符号链接或错误身份读出别人的文件。它保护运行系统的安全，也保护研究者不被无归属的内部细节淹没。

这些不变量比某个默认数字更稳定。默认轮数可以从十改成五，并发可以从四改成二，模型可以更换；只要不变量仍然成立，机制的骨架就没有被破坏。

## 第九十四章　当你需要诊断一次运行时

诊断不要从“模型为什么这么想”开始，而应先问“流程现在在哪里”。先读取摘要、Session 和候选目录，确认 Launch、Round、Session 和候选数量；再看状态是否与目录匹配；最后才进入阶段日志和外部后端的细节。这样可以先排除输入、恢复和编排错误，再讨论科学结果。

如果状态卡在入口，检查任务类型、prompt、参考代码和 Launch 快照；如果卡在生成或反思，检查运行时、Agent 注册表、Session JSON 和模型调用日志；如果卡在外部数据，检查 Connector 清单、`acquisition_events`、搜索服务和并发限制；如果卡在实验，检查工作台、GPU 分配、后端日志和 `run_N/final_info.json`。
...
任务目录存在，却没有 `prompt.json`。普通任务的 Launch 准备在复制输入时抛出缺失错误，外层不会建立一个让模型自由发挥的空 prompt。科学任务若缺少 `task_info.json` 或清单无法读取，也会在标准化边界停下；所有错误都留在输入门口。

如果有人手工在 Launch 目录创建一份空文件再恢复，系统仍应读取它的内容并让后续校验发现字段不足；它不会因为文件名存在就把空白当成有效任务。这个例子说明“存在性”和“有效性”是两道不同的门。

故障修复是补齐原始任务或修复标准化材料，然后建立新的 Launch 或在明确允许的恢复路径上继续。旧的失败记录保留，用来解释为什么第一次没有开始。删除失败目录只会让下一位读者失去输入链索引。
...
透明度也有成本。保存阶段日志、工具轨迹和每次失败会占空间，但删除它们会让科学复盘失去上下文。底层读取还需要做一次身份和路径检查，增加实现复杂度，却避免把无归属的内部细节混进别的 Launch。一个成熟系统不会把透明与安全看成互斥，而是为不同读者维护不同层的证据。

## 第一百章　当系统终于可以被讲清楚

如果把整个 Discovery 压缩成一句话，可以说：它把一个经过确认的研究输入，交给一支状态化的多角色队伍，产生并筛选候选，再把候选复制到隔离工作台接受外部执行，最后把成功、失败、指标、记忆和下一轮起点都写入可恢复的事实目录，并把这份目录交给唯一的 `paper` Run。
...
卡片加厚不是把所有原文堆进去。编排器选择结构化字段，阶段日志和工具轨迹保存较长观察；Idea 的 `critiques` 不是一段长对话，`evidence` 不是所有搜索结果，`score` 不是模型的情绪。字段的压缩让后续 Agent 能够消费，也让恢复不会被一段巨大文本拖垮。
...
工具结果要保留输入和输出的对应关系。只记录“查过文献”不足以解释生成员为何得到某条证据；只记录模型最后答案又无法知道它是否真的使用了外部资料。阶段日志和工具轨迹提供更长的回声，Idea 只接收经过角色整理的结构化结论。

工具错误也有传播级别。一个可选工具失败，角色可以在没有它的情况下继续；一个必需工具失败，阶段处理器可以把 Session 置为 ERROR。配置和角色定义决定边界，编排器不能为了追求成功率把所有工具错误都吞掉。

## 第一百零六章　等待并不等于没有人工作

AWAITING_FEEDBACK 的房间里可能没有模型调用，却有外层驾驶员、离线反馈文件和持久化 Session 在工作。状态写入以后，运行器可以退出，下一次启动再把反馈带回来。等待被设计成可持续的事实，而不是内存里的 sleep。
...
第一条线是输入线：任务目录或科学任务材料，经过标准化和 Launch 快照，形成 Task。第二条线是状态线：INITIAL、GENERATING、REFLECTING、EXTERNAL_DATA、EVOLVING、RANKING、AWAITING_FEEDBACK，再进入方法阶段和 COMPLETED 或 ERROR。
...
第五条线是观察线：状态回调、阶段日志、Session JSON、轮次记录、候选工作台和论文 Run 把发生过的动作分层保存。第六条线是记忆线：Task Memory、IdeaGraph、在线记忆、经验库和提示演化把过去带进未来，但不改变当前任务的身份。
...
工程师要问的是：进程被杀、机器重启、外部服务超时以后，系统能不能从磁盘继续，而不是从零生成一个看似相同的运行。答案散落在几个位置：Launch 快照、Session JSON、轮次摘要、运行器标记、阶段日志和候选工作台。
...
磁盘管理也必须尊重历史。候选工作台、run 目录和阶段日志可能很大，但它们承载失败和重放证据；共享经验库和来源材料还可能被多个 Launch 引用。清理策略应按引用和终态设计，不能用“删除最旧目录”这种粗糙规则破坏恢复链。

## 第一百一十六章　给维护者：问它是否守住底层边界

维护者首先要确认，状态、阶段和产物都来自同一组持久化事实，而不是某条日志或某个进程内缓存。任何观察都应能回到 Launch、Session、候选工作台、运行器标记或 `paper` Run；如果找不到来源，就先标成待确认。

一个可靠的维护路径可以把状态讲成故事：“正在生成候选”“等待反馈”“正在验证 3/5”“本轮已完成，准备交给 PaperOrchestra”。这些句子只是对底层门牌的翻译，不是新的事实。原始状态、Session ID、轮次、错误和证据路径仍然必须保留。

重启后，维护者应重新读取 Launch、Session 和运行器标记，而不是沿用进程退出前的最后一帧。若已经完成，恢复只能复用既有事实；若仍有可恢复材料，就从明确的阶段继续；若目录不完整，就停在需要处理。底层先保证可解释，其他观察者再决定如何呈现。
...
审计不需要把每个 token 公开。它需要知道某个候选为什么进入实验、实验使用什么基线、指标从哪个文件读取、下一轮为什么采用某条代码。阶段日志可以保留更细的过程，审计者先从证据索引和摘要开始，再按需回到运行目录。
...
测试者不只运行一条 happy path。要重复启动和恢复同一个 Launch，读取不存在的 artifact，读取 `../` 路径，创建没有 prompt 的普通任务，创建没有 checklist 的科学任务，让当前运行配置与恢复摘要冲突，再验证 Paper Handoff 重入不会创建第二个 `paper` Run。

每个边界都要有预期的状态和错误类型。无效输入在启动前拒绝，未知后端在候选执行时失败，单 Idea 的实验异常不应吞掉其他结果，终态不允许改变，论文失败不回写 Discovery 历史，证据路径不越界。测试名称最好直接说出这些不变量，以便未来修改者理解为什么断言存在。

## 第一百一十九章　给后来要扩展系统的人

新增一个 Agent 时，要确定它需要什么上下文、返回什么结构、失败影响哪一层、结果写回 Idea、Session 还是阶段日志。新增一个实验后端时，要遵守工作台、日志、GPU、成功标志、性能和错误的结果合同。新增一种任务类型时，要在输入标准化、代码基线、输出目录和摘要字段上写清楚它与 auto、sci 的差别。

扩展不能只增加一个 if 分支。分支会穿过入口、配置、运行时、AgentFactory、ExperimentRunner、结果统计、恢复扫描、PaperOrchestra 交接和测试。深模块的做法是把差异封装在边界内，再让外层继续消费同一份合同；否则故事会越来越像一份互相覆盖的例外清单。
...
本书写到 `paper_orchestra_runs/paper/`，并不意味着仓库里的 Discovery 或 PaperOrchestra 只有这么多文件；它意味着故事已经沿着底层交接走到最终产物。读者如果要继续深入，可以以摘要为索引，沿着 Session、Idea、工作台、run 目录、阶段日志、记忆库和 `paper` Run 逐层展开。每一层都有更细的实现细节，但不会改变前面这条因果链。

后续实现可以加入新的模型适配器、新的外部后端、新的实验指标、新的证据字段或新的论文消费者，只要它们遵守输入、状态、隔离、证据、恢复和安全这几条骨架，整个系统仍然可以被同一种方式讲清楚。真正的全景不是把所有名称罗列一遍，而是让新名称有地方落座。
...
故事里，接待员不问模型“你觉得这是什么任务”，只检查任务目录、文件存在性和可读取的输入。普通任务找到 prompt，科学任务找到 task_info；任何事实不成立，研究留在门外。卡片的关键句是：先确认你手里拿的东西，再决定往哪条路走。
...
故事里，值班者只从选定 Launch 的根目录取证，不把另一个运行的文件、绝对路径或符号链接逃逸混进当前账本；路径穿越和身份错配都被挡住。卡片的关键句是：透明要有边界，安全也要有证据。
...
**Launch**：一次外层运行的隔离边界。它拥有自己的输入快照、配置快照、Session、实验结果、摘要和论文交接目录。
...
**idempotency key**：一次外部动作的重试身份。底层恢复和论文交接也需要等价的 Launch/Run 身份，防止重复创建同一阶段。

**request fingerprint**：根据动作不可变输入计算出的指纹。它帮助系统区分“同一次重试”和“复用同一身份做另一件事”。

**observation**：从 Session、摘要、运行器标记和目录产物重建的状态投影。它服务于恢复和诊断，不替代事实源。

**artifact**：可以被定位和核对的 Launch 产物。路径绑定根目录，日志、指标和论文材料各自保留自己的证据职责。

**path traversal**：通过 `../`、绝对路径或符号链接逃出选定 Launch 根目录的读取尝试。Discovery 应拒绝它。

**reconcile**：把磁盘、运行器标记和服务器索引重新对账的过程。它让状态观察不会永久停留在旧内存。

**PaperOrchestra boundary**：Discovery 与论文流水线之间的单向交接。Discovery 在摘要和原生产物上封存事实，PaperOrchestra 在固定的 `paper` Run 中消费并生成论文，不回写 Discovery 历史。
...
统一模型入口在 `vegapunk/mas/models/unified_runtime.py`，工具回路在 `vegapunk/mas/agents/tool_loop.py`，会话文件记忆在 `vegapunk/mas/memory/memory_manager.py`。这几处分别对应能力、外部动作和恢复骨架；阶段日志与候选工作台承载过程证据。

论文交接与运行服务在 `vegapunk/paper_orchestra/service.py`、`vegapunk/paper_orchestra/` 和 `launch_discovery.py` 的 `_handoff_to_paper_orchestra` 附近；Connector 证据边界在 `vegapunk/mas/workflow/external_data.py` 与 `orchestration_agent.py`。测试目录中的 Launch、Connector、PaperOrchestra 和 runtime 测试，是这些底层边界的可执行注释。

读代码时可以使用五个问题作导航：这个对象从哪里来？它被谁调用？它写回什么？失败传播到哪里？重启后凭什么重新得到它？只要每个关键对象都能回答这五问，代码细节就不会变成散乱的名字。

# 附录 D　终稿验收清单

长度方面，正文与附录合计应达到至少十万中文字符或等量字符规模；章节扫描应覆盖入口、任务规范化、外层循环、Session 状态、角色、运行时、工具、Connector 证据、实验、指标、失败、恢复、记忆、PaperOrchestra 交接、产物、测试和术语表。书中不再使用大段 Python 代码块，机制含义通过故事骨架、条件关系和字段血缘表达。

边界方面，PaperOrchestra 作为 Discovery 完成后的固定 `paper` Run 接入，书中只讲启动、输入材料、运行目录、终稿产物和失败隔离，不展开其内部章节、提示词或版式审查。任何章节若把论文输出写成 Discovery 内部字段，或把 Paper 失败写成 Discovery 轮次失败，都应回到这一边界修订。
...
第四，门牌不是外部显示标签，而是 WorkflowState 的状态合同。第五，工作台不是容器产品的专名，而是每个候选独立复制的代码目录。第六，接力棒不是一个神秘分数，而是最佳结果经过代码、指标和必要产物同步更新后的 baseline。
...
科学任务目录里有 `task_info.json`，却没有可读取的 checklist。标准化程序把清单记为缺失但不伪造通过，把任务说明、数据清单和空约束写进合成 prompt，并让后续验收知道“没有清单”这个事实。输入材料不完整可以继续，但不能被叙事改写成完整。

## 剧场二：恢复路径被同时触碰两次

两个恢复进程同时发现同一 Launch 还没有写出最终摘要。第一个进程拿到目录锁，读取已完成轮次并登记下一步；第二个进程在锁内看到相同的 Launch 身份，只能复用或等待，不能再创建第二套 Session。后台因此只有一条连续历史，重试不会变成重复运行。

## 剧场三：任务目录在恢复前被修改

第一轮已经把 prompt 复制进 Launch，随后任务目录里的原始提示被编辑。恢复程序仍然先读 Launch 内的快照，不把当前目录的文字偷偷套进旧摘要；如果研究者确实要用新任务，就建立新的 Launch。系统宁愿多一次明确分叉，也不把两个研究问题混在同一条历史里。
...
编排进程重启后，内存里的 waiting 事件消失，但 Session JSON 和 Launch 状态仍在。下一次运行从磁盘加载并确认 `AWAITING_FEEDBACK`，外层驾驶员读取离线 feedback，Interface 追加历史并恢复。没有任何 live await 被假定仍然存在，等待状态靠持久化事实重建。

## 剧场十五：候选工作台出现越界符号链接

候选目录里有一个看似普通的链接，实际指向 Launch 根目录之外的文件。产物解析器先规范化路径并检查真实目标是否位于允许根目录，发现逃逸后拒绝把它接入证据。路径安全边界在解析阶段，而不是在任何展示阶段。
...
**为什么等待反馈也要写状态？** 因为进程可以退出、机器可以重启，只有持久化状态才能让外部意见在未来继续同一条走廊。

**为什么回调失败不改变 Session 状态？** 回调属于观察层，研究流程属于事实层；把旁观者的故障传回主流程会让记录问题变成研究失败。

**为什么阶段日志和结构化状态要分开？** 日志适合保留过程和轨迹，JSON 适合恢复和条件判断；一份文件无法同时满足两种形态而不变得笨重或模糊。
...
**为什么指标不能直接从 runner.log 读取？** 运行器日志常含命令、路径和调试数据；指标必须回到 `final_info.json`，内部诊断材料只能解释过程，不能替代事实源。

**为什么 artifact 读取必须绑定 Launch？** 同名任务和候选可能存在于多个运行，先绑定根目录才能防止跨 Launch 读取。

**为什么要拒绝非有限数字？** 无限或 NaN 会破坏 JSON 消费、排序和下游读取，不能成为研究指标。

**为什么恢复必须绑定 Launch 身份？** 同名任务和候选可能存在于多个运行，先绑定根目录才能防止跨 Launch 读取或把两条历史拼在一起。

**为什么来源材料要留在 Launch 的输入快照旁边？** 快照让恢复知道当时看见了什么，原始目录让研究者仍能回查来源；两者缺一不可，也不能用一段日志代替。

**为什么终态不允许停止或恢复？** 完成或失败已经是历史事实，动作只能改变运行而不能改写历史；只读访问仍然开放。

**为什么报告生成成功不代表科学成功？** 报告只是把候选写成文本，未必有实验或有效指标；它的成功字段应和实验性能分开解读。

**为什么 PaperOrchestra 只以 `paper` Run 接入？** 固定运行身份和目录让论文重入可复用、失败可隔离，也让它不能悄悄创建第二篇或改写 Discovery 历史。
...
Markdown 转 EPUB 时，书名、卷标题、章标题应进入导航目录；章节之间保留自然换页，但不要把每一段拆成一个独立阅读单元。正文的引用、行内代码和表格需要使用适合中文排版的字体和行距，避免技术对象名被拆在奇怪的行尾。
...
电子书不应把附录隐藏在正文之后的无名章节里。机制卡帮助快速复述，术语表帮助定位，源代码地图帮助回仓库，问答帮助确认理解，验收清单帮助检查交付。它们共同把一部长篇叙事变成可以反复使用的研究工具。
...
INITIAL → GENERATING：状态变化是一个可观察事件，不是一个外部进度标记；失败时要写 error，不用空 Idea 伪装继续。
...
rounds + sessions + results → discovery_summary：摘要保存可恢复横截面，不把全部阶段日志复制成巨型文件。

task directory → prompt snapshot：入口确认任务类型后，把普通 prompt 或科学任务合成 prompt 放进 Launch，恢复不重新猜输入。

Launch identity → resume admission：恢复绑定 Launch、Session 和已完成轮次，重复进入复用同一事实，不制造第二条历史。

source files → input snapshot：任务目录、配置和模型目录在 Launch 周围留下快照，后来变化的全局设置不能污染旧运行。

Launch artifacts → evidence coordinates：文件先绑定 Launch 或候选工作台根目录，再允许读取；绝对路径、`../` 和符号链接逃逸被拒绝。

completed 或 failed → read-only history：终态保留摘要、错误和产物，恢复只能从明确边界继续，历史不能被重写。

阶段日志 + Session JSON → 可核对事实：过程叙述留在执行器自己的记录里，结构化状态保留恢复所需字段，原来的事实不被覆盖。
...
再确认覆盖范围：从任务输入开始，读者能找到普通与科学任务分流、Launch 建立、prompt 和配置快照、模型运行时、AgentFactory、工具循环、Session 状态、Idea 谱系、反馈插槽、Connector 证据、方法开发、实验工作台、run_0 与 run_N、指标、资源、失败、增量基线、长期记忆、产物安全、恢复、测试、摘要终点和 PaperOrchestra 交接。
...
## 交接十：PaperOrchestra 将接到什么材料

列出摘要、候选工作台、指标、报告、阶段日志和 `paper` Run 的位置。说明哪些是 Discovery 原生产物，哪些是论文阶段新生成的 TeX、PDF 和运行记录；如果论文读取失败，先检查 Launch 绑定、原生产物是否完整和 `paper` Run 是否已经存在，不要重新猜一份候选。
...
然后看 log 和阶段记录。日志提供阶段、后端、GPU、超时和错误，阶段记录提供模型与工具的长观察。它们解释为什么代码变成当前样子，但不应被直接当作指标来源。指标仍应回到 final_info，状态仍应回到 Session 和摘要，过程材料负责连接因果。
...
任务目录里的 prompt 在上一轮实验后被修改，同时最佳代码也已经被增量模式前移。下一次启动不能只问“代码从哪里来”，还要问“任务输入是否还是同一个”。代码路径由 baseline 逻辑决定，研究文字由 Launch 内的 prompt 快照决定；若两者不再属于同一条研究命题，最安全的动作是建立新的 Launch，而不是把新问题套在旧代码上。
...
并行实验还未全部结束时，值班者可能读取中间目录。底层观察应区分已完成、运行中、失败和等待中的候选，而不是把“尚未出现 final_info”误当作失败，也不要把已退出的进程继续记成运行中。摘要只有在轮次封存后才写最终成功数，中间观察和最终摘要粒度不同，却都来自同一组工作台事实。
...
外部后端可能在一次运行失败后被自动重试，编排器也可能因为进程重启重新进入恢复。两种重试的对象不同：后端重试可以产生新的 `run_N`，恢复重试应该指向同一个 Launch 状态。日志和结果要能区分“同一动作被重新请求”和“候选在同一工作台里进行了新的实验尝试”。

如果一个恢复路径触发了第二个外部进程，而第一个进程其实仍然在运行，两个进程可能同时修改同一工作台。目录锁和运行器标记必须在恢复入口挡住这个竞态；工作台内部的 run 编号不能替代 Launch 层的动作锁。
...
`discovery_summary.json`、`final_info.json` 和报告需要被 PaperOrchestra 读取，`runner.log`、traceback 和后端命令则服务于工程诊断。最稳妥的方式不是复制两套互相漂移的文件，而是让两类读者指向同一个 Launch 根目录和同一份事实：论文材料只取确定性原生产物，内部诊断按需读取更详细的日志。

如果论文输入把错误文字完全删掉，PaperOrchestra 可能失去解释失败的上下文；如果把内部命令和绝对路径当成科学材料，又会扩大证据边界。可以把错误类别、阶段、候选和建议动作写入结构化材料，把敏感细节留在内部日志。透明度的目标是帮助下一阶段采取正确动作，而不是让所有内部字节都变成论文事实。

## 版本升级和旧摘要同时存在

当代码升级了字段名或状态枚举，旧 Launch 仍然可能被读取。向后兼容层可以把旧字段映射到当前对象，再按新格式写出，但必须保留原始身份和版本线索。恢复成功不代表旧状态与新状态完全等价，摘要和日志应显示发生过兼容转换。

如果状态枚举从一个房间拆成两个房间，旧摘要可能只能定位到较粗的门牌。系统可以从 Session JSON、阶段日志和目录产物推断更细位置，但推断出来的部分应标为恢复观察，而不是伪装成旧运行曾经写下的事实。版本迁移越诚实，未来审计越容易。

## 交叉检查的结论

复杂系统的难点往往不在单个模块，而在两个模块同时改变时谁拥有解释权。输入与代码、模型与提示、反馈与经验、并发与观察、指标与清单、记忆与隔离、重试与恢复、旧格式与新逻辑、论文材料与内部诊断、版本升级与历史摘要，都是会产生这种交叉的地方。
