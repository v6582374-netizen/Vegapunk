...
// Application update prompt (release desktop shell only - the browser build never renders this).
...
// the first seconds). The full artifact is downloaded only after the user accepts
// the update.
...
type Phase = "ready" | "installing" | "error";
...
export function UpdateBanner() {
  ...
  // Do not reset an in-flight or failed install when the periodic check reports the
  // same release again.
  const offered = useRef<string | null>(null);

  const offer = (u: UpdateInfo) => {
    if (u.version === dismissed.current || u.version === offered.current) return;
    offered.current = u.version;
    setUpdate(u);
    setPhase("ready");
  };

  useEffect(() => {
    if (!isTauri() || !isUpdaterEnabled()) return;
  ...
  return (
    ...
    >
      ...
      <div className="text-[12px] text-muted mt-0.5">
        Vegapunk v{update.version} is ready to download.
      </div>
      {phase === "error" && (
        <div className="text-[11.5px] text-warnInk mt-1.5">
          The update could not be installed - it will be offered again next launch.
        </div>
      )}
      <div className="flex items-center gap-2 mt-2.5">
        ...
        >
          {busy ? "Downloading…" : "Download and restart"}
        </button>
        <button
