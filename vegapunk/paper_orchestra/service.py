...
async def run_paper_orchestra(
    *,
    ...
    try:
        ...
        catalog_path = _resolve_catalog_path(vegapunk_config)
    ...
    selection = await _optional_candidate_selection(
        ...
        run_dir=run_dir,
        vegapunk_config=vegapunk_config,
        runtime=runtime,
    )
...
def _resolve_catalog_path(vegapunk_config: dict[str, Any]) -> Path:
    raw_path = vegapunk_config.get("model_catalog_path")
    if raw_path is None:
...
def _write_runtime_config(
    *,
    ...
    path = run_dir / "vegapunk_runtime.json"
    path.write_text(
