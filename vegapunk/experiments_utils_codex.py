...
def _generate_report_with_codex(codex_runner, folder_name, final_run, final_iter, completed=True):
    ...
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    backend_label = getattr(codex_runner, "backend_label", "Codex CLI")
    logger.info(f"[{timestamp}] Requesting {backend_label} to generate experiment report for {folder_name}")
...
"""
    ...
    logger.info("Sending report generation request to %s", backend_label)
    codex_output = codex_runner.run(report_prompt, cwd=folder_name)
    logger.info(f"{backend_label} report generation completed (output length: {len(codex_output)})")
