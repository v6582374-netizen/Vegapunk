...
from autoraters.prompts.sxs_quality_prompts import sxs_lit_review_quality_system_prompt

# Global lock for file writing
FILE_LOCK = threading.Lock()


def rate_sxs_lit_review(
    paper1_text: str, paper2_text: str, model_name: str = "gemini-3.1-pro-preview"
):
    max_retries = 5
    cur_try = 0
    succeed = False

    prompt = f"Paper 1:\n{paper1_text}\n\n---\n\nPaper 2:\n{paper2_text}\n\nTask: Compare the literature review quality of these two papers side by side."
        ...
        try:
            if "gemini" in model_name:
                response_dict = call_llm_with_text_prompt(
                    ...
                    generation_configs={
                        "system_instruction": sxs_lit_review_quality_system_prompt,
                        "temperature": 0.0,
                    },
                )
                succeed = True
            ...
            print(
                f"Error processing SxS: {e}. Retrying [{cur_try + 1}/{max_retries}]...",
                flush=True,
            )
            cur_try += 1
...
def _process_and_save_single_sxs(
    ...
    """
    Helper function to run inside the thread. Rates SxS lit review twice (both orders) and aggregates.
    """
    try:
        paper1_text = load_paper(paper1_path, min_size=100)
    except Exception as e:
        print(f"Error reading paper 1 {paper1_path}: {e}")
        return paper_id, False

    try:
        paper2_text = load_paper(paper2_path, min_size=100)
    except Exception as e:
        print(f"Error reading paper 2 {paper2_path}: {e}")
        return paper_id, False

    rating1 = rate_sxs_lit_review(paper1_text, paper2_text, model_name=model_name)
    rating2 = rate_sxs_lit_review(paper2_text, paper1_text, model_name=model_name)
    ...
    print(
        f"✅ [{paper_id}] Finished rating SxS Lit Review (Double). 5pt: {category_5pt}, 3pt: {category_3pt}",
        flush=True,
    )
    return paper_id, True


def perform_sxs_lit_review_batch(
    ...
    exp1_name: str,
    exp2_name: str,
    output_folder: str = "./logs/sxs_lit_review_quality",
    model_name: str = "gemini-3.1-pro-preview",
):
    print(f"Starting Enhanced SxS Lit Review batch execution...", flush=True)
    ...
    output_json_path = os.path.join(
        output_folder,
        f"sxs_lit_review_enhanced_{model_name}_{exp1_name}_vs_{exp2_name}_{conference}.json",
    )
    ...
    print("\n" + "=" * 50, flush=True)
    print("ENHANCED SxS LITERATURE REVIEW QUALITY EVALUATION SUMMARY", flush=True)
    print("=" * 50, flush=True)
